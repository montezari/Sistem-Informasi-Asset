...
if(strtoupper($_POST["FormAction"])=="SIMPAN"){
  ...
  if($_POST["fgen"]=="0"){
	...
	$rs = $conn->Execute($sql);
	while(!$rs->EOF){
	  $sql  = "INSERT INTO tm_asset (cIdAsset,cKdAsset,cIdSPA,cNoSPA,cIdSPB,cNoSPB,cKdBarang,cKdSatuan,dTglAsset,cAktif,cIdGenerate,dTglGenerate, ";
	  $sql .= "cUserEntry,cDateEntry,cUserEdit,cDateEdit) values ( ";
	  $sql .= "'".$rs->fields["cIdAsset"]."','TEST',".$rs->fields["cIdSPA"].",'".$rs->fields["cNoSPA"]."',".$rs->fields["cIdSPB"].",";
	  $sql .= "'".$rs->fields["cNoSPB"]."','".$rs->fields["cKdBarang"]."','".$rs->fields["cKdSatuan"]."',CURRENT_TIMESTAMP,'1','".$rs->fields["cIdGenerate"]."',";
