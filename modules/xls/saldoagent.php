...
class TExcelRptSaldoAgent extends TExcel {

  function doExportXls(){
  global $conn, $config, $LABEL;  

	$sql  = "CALL sp_saldo_agent('$_REQUEST[kdagent]','$_REQUEST[kdcurr]','$_REQUEST[cmbbln]','$_REQUEST[cmbtahun]'); ";
	$rs = $conn->Execute($sql);
	//print_r($rs);

	if($rs->RecordCount()>0){
	  $xlstpl = new TExcelTemplate("xls_template/saldoagent.xls");
	  ...
	  */
	  ...
	  header('Content-Disposition: attachment;filename="saldo_agent_'.$dtfile.'.xls"');
