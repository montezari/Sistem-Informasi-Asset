...
class TExcelRptInventarismutasi extends TExcel {
  ...
  global $conn, $config, $LABEL;  
	...
	if($rs->RecordCount()>0){
	  $xlstpl = new TExcelTemplate("xls_template/inventarismutasi.xls");
	  ...
	  header('Content-Disposition: attachment;filename="Laporan_Mutasi_Inventaris_'.$dtfile.'.xls"');
