...
class TExcelRptRekapSMU extends TExcel {
  ...
  global $conn, $config, $LABEL;  
	...
	$sql .= "WHERE mu.ckdagent = '$_REQUEST[kdagent]' AND mu.ckdcurrency = '$_REQUEST[kdcurr]' ";
	...
	if($rs->RecordCount()>0){
	  $xlstpl = new TExcelTemplate("xls_template/rekapsmu.xls");
	  ...
	  header('Content-Disposition: attachment;filename="rekap_smu_'.$dtfile.'.xls"');
