...
class TExcelRptInvoiceList extends TExcel {
  ...
  global $conn, $config, $LABEL;  
	...
	$sql .= "WHERE inv.ckdagent = '$_REQUEST[kdagent]' AND inv.ckdcurrency = '$_REQUEST[kdcurr]' ";
	...
	if($rs->RecordCount()>0){
	  $xlstpl = new TExcelTemplate("xls_template/invoicelist.xls");
	  ...
	  $sumrow = $row+$awalrow;
	  $objWorksheet->setCellValue("M".$sumrow,'=SUM(M'.$awalrow.':M'.$bykrow.')');
	  ...
	  header('Content-Disposition: attachment;filename="invoice_list_'.$dtfile.'.xls"');
